...
struct AppConfig {
    ...
    // [Swatch Background] CanvasColor==5: 0-2=built-in checkerboards, 3-8=custom RGBA, 9=adaptive black/white/gray
    static constexpr int MAX_SWATCH_COLORS = 10;
    int SwatchColors[10][4] = {
    ...
    // --- Thumbnail Panel (Sidebar) — Legacy single config, kept for migration ---
    int ThumbnailPanelSide = 0;    // 0=Right(默认), 1=Left, 2=Off, 3=Bottom
    float ThumbnailPanelWidth = 180.0f; // 用户拖动后的面板宽度（逻辑px，持久化）

    // --- PDF Page Thumbnail Panel (independent config) ---
    int PdfThumbPanelSide = 1;    // 0=Right, 1=Left(默认), 2=Off, 3=Bottom
    float PdfThumbPanelWidth = 140.0f; // 面板宽度（逻辑px，持久化）
    float PdfThumbPanelHeightBottom = 100.0f; // 底部模式高度

    // --- Image List Thumbnail Panel (independent config) ---
    int ImageThumbPanelSide = 3; // 0=Right, 1=Left, 2=Off, 3=Bottom (默认底部)
    float ImageThumbPanelWidth = 140.0f; // 面板宽度（逻辑px，持久化）
    float ImageThumbPanelHeightBottom = 100.0f; // 底部模式高度
