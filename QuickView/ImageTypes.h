...
struct RawImageFrame {
    ...
    struct SvgData {
        ...
        // [Viewport Crop] 真实内容包围盒，由异步光栅线程在首次解析树成功后
        // 计算一次。注意：这是 resvg 树坐标（usvg 把根 viewBox 原点烘焙进
        // 树空间，内容坐标 = 用户坐标 - viewBox原点），使用方必须加回
        // viewBoxX/Y 换算成 SVG 用户坐标。0=未计算 1=可用 2=计算失败。
