...
// 手段：先试官方 API SetAppAsDefault（在其可用的系统上最干净）；实测部分
// Win10 19045 上一律 E_FAIL，此时本地计算合法 UserChoice 哈希直接写入——这是
// 防不住「照片」AppX / Edge 回收的唯一手段（回收只覆盖缺失/无效的 UserChoice，
...
bool SettingsOverlay::RegisterAssociations() {
    ...
    // 2. Set DefaultIcon to the QuickView executable so file associations show
    // the app icon instead of the generic "unknown type" shell32.dll icon.
    SafeRegSetString(HKEY_CURRENT_USER, L"Software\\Classes\\QuickView.Image\\DefaultIcon", NULL, (exePathStr + L",0").c_str());
    ...
    SafeRegSetString(HKEY_CURRENT_USER, L"Software\\Classes\\QuickView.Vector\\DefaultIcon", NULL, (exePathStr + L",0").c_str());
    ...
    //
    // This step is decoupled from the user's FileAssocExts (open-with) choice:
    // - Thumbnail provider: ALWAYS active for all 9 vector formats
    ...
    // 8c. [默认应用接管/迁移] 见 ReassertDefaultTakeover()（同一逻辑也在每次
    // 启动空闲时执行，用于从「照片」AppX 等手里抢回被回收的关联）。
    ReassertDefaultTakeover();

    // [Fix] Register Thumbnail Provider COM DLL for ALL image formats QuickView
    // renders. The DLL is a thin shim that delegates rendering to
    // "QuickView.exe --thumbnail" (which draws the type badge on top).
    {
        ...
        if (GetFileAttributesW(dllPath.c_str()) != INVALID_FILE_ATTRIBUTES) {
            ...
            // - QuickView.Image: defensive fallback for formats whose UserChoice
            //   hash points here (Windows may redirect .svg UserChoice → Image)
            std::wstring shellexKeyVec = L"Software\\Classes\\QuickView.Vector\\ShellEx\\" + std::wstring(thumbnailIID);
            ...
            // image format QuickView supports through our provider regardless
            // of which app (WPS/Edge) holds the default-app UserChoice hash.
            // WPS/Edge do NOT register any .ext-level ShellEx, so this is safe and wins.
...
void SettingsOverlay::UnregisterAssociations() {
    ...
    for (const auto& ext : QuickView::SUPPORTED_EXTENSIONS) {
        ...
        // Clear .ext default value if it points to QuickView.Image
        {
            HKEY hExtKey;
            std::wstring extKeyPath = L"Software\\Classes\\" + extStr;
            if (RegOpenKeyExW(HKEY_CURRENT_USER, extKeyPath.c_str(), 0, KEY_READ | KEY_WRITE, &hExtKey) == ERROR_SUCCESS) {
                ...
                if (RegQueryValueExW(hExtKey, NULL, NULL, &valType, (LPBYTE)curVal, &valLen) == ERROR_SUCCESS) {
                    ...
                    if (curStr == L"QuickView.Image" || curStr.rfind(L"QuickView", 0) == 0) {
                   ...
                   L"Software\\Classes\\CLSID\\{DAA561A2-0EEA-478F-9C2E-DBC41B59056B}");
    {
        ...
        for (const auto& ext : QuickView::kThumbnailExts) {
            std::wstring extStr(ext);
...
void SettingsOverlay::BuildMenu() {
    ...
    */
    ...
    // [Removed] Info Panel settings — title bar always shows info, hover for details
    ...
    // Thumbnail server (Shell 缩略图常驻服务) — 高级性能参数
    tabAdvanced.items.push_back({ L"缩略图渲染", OptionType::Header });

    SettingsItem itemThumbThreads = { L"并行线程数", OptionType::Slider, nullptr, &g_config.ThumbnailThreads };
    itemThumbThreads.minVal = 1.0f;
    ...
    itemThumbThreads.onChange = [](SettingsOverlay* overlay, [[maybe_unused]] SettingsItem* item) {
        ...
        SaveConfig();
        QuickView::KillThumbnailServer();
        if (overlay && overlay->m_hwnd) InvalidateRect(overlay->m_hwnd, nullptr, FALSE);
    };
    ...
    tabAdvanced.items.push_back(itemThumbThr);
