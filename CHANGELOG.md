...
## [6.30.18] - CAD 解析缓存改为字节预算 + LRU，内存不再无界增长
**Release Date**: 2026-09-01

### 🧹 Changes
- **CAD 解析共享缓存（`ImageLoader.cpp`）的容量策略从"满 24 条整表清空"改为 64MB 字节预算 + LRU 逐条淘汰**:
  - 为什么改：一张工程图的 SVG 就有数 MB（实测 DWG 达 7.15MB），按条数封顶意味着最坏情况要累积到约 170MB 才一次性全清（24 条 × 7MB 的算术推算），而且整表清空会把**正在被使用的条目**连同刚解析完的结果一起丢掉，翻文件夹时容易反复全量重解析。
  - 怎么做：每条缓存记账自己的 SVG 字节数和最近使用序号，超出 64MB 预算时按 LRU 逐条淘汰。**仍在解析中的条目不参与淘汰**（`bytes == 0`），否则并发挂接的等待者会拿不到结果；若某一刻在途条目本身就超预算，允许暂时超出，等它们完成后立即收敛。
  - 淘汰只影响"下次打开要不要重算"，不影响任何渲染输出；进程退出时缓存整体释放（本就是纯内存结构，不落盘）。

### 📊 实测（缩略图服务连续喂 12 份 DWG 副本，每份缓存约 7.4MB）
- 服务基线 13MB → 每新增一份涨约 7.4MB → **第 9 份起稳定在 92MB 不再增长**，预算生效。
- 淘汰确认：重问最早那份（已被挤出）耗时 **2284ms**，即完整重新解析；重问最新那份 **620ms**，即正常命中。快路径没有被误伤。
- 回归确认：`--export-png` 四种格式的 `rc`、耗时与非白像素数全部与 6.30.17 **逐项相同**（DWG 158731 / DXF 240981 / 合身款 1592242 / PLT 158962），渲染输出零变化。

### ⚠️ 仍未处理
- `%TEMP%` 下 `qvstream_*` 落盘临时文件在累积（本机实测 676 个 / 17.1GB，跨 8-21 至 9-01，`.tif` 与 `.pdf` 占九成）。那是缩略图处理器把 Shell 的 `IStream` 转成真实文件的中转，属**磁盘**占用而非本次的内存问题，清理逻辑存在但会静默失败，需单独定位。

## [6.30.17] - 修复 DWG 预览打不开：样条离散失控（SVG 68MB → 7MB）
**Release Date**: 2026-09-01

### 🐛 Bug Fixes
- **修复：`DNTS30062(1).dwg` 打开后一片空白 / 被判为加载失败**:
  - 根因：`DwgRenderer::Render()` 里 SPLINE 的离散策略写死成"每个控制点采 20 个点"（`numSamples = max(100, num_ctrl_pts * 20)`）。工程图/向量化底图的样条控制点总数动辄数万，乘以 20 后离散出上百万个顶点，SVG 膨胀到 **68MB**、渲染耗时 **13.7 秒**，直接超过 `kDwgTimeoutMs = 8000` 的加载超时——主视图与资源管理器缩略图同时判死，用户看到的就是"这个 DWG 打不开"。HATCH 的样条边界另有 `max(50, n * 10)` 同款问题。
  - 修复：改为**按曲线自身尺寸的相对弦高自适应细分**（`FlattenSpline` / `FlattenSegment`）。先在每个 knot 区间两端点之间取中点，测该点到弦的法向偏差；偏差小于容差即视为平直、一个区间只出一个点，否则递归二分。容差取**控制点包围盒对角线 × 1e-4**（凸包性质保证包围盒是曲线尺寸的安全上界），因此纯直段样条退化为 2 个点、弯曲处自动加密，放大若干倍仍看不出折线化。
  - 三个关键细节：
    1. **容差必须是纯几何量**。早期版本取 `max(包围盒对角线, 参数区间长度) × 相对容差`，把参数空间混进了长度容差——"参数跨度大而绘图尺寸小"的样条会被当成完全平直处理，真实曲率整段丢失。改为只用包围盒对角线后 DNTS 输出 6614KB → 7150KB，多出的 500KB 全是被错误抹掉的弯曲细节。
    2. **深度上限 5 是代价闸门**。实测深度 6 与深度 5 输出**逐字节一致**（容差已收敛），继续放开深度只会让向量化底图这类锯齿样条一路细分到上限。单条样条另有 6000 点硬上限防御畸形数据。
    3. **平直性只看中点会漏判**。带拐点的 S 形区间中点恰好落在弦上、两端却外鼓，故在 1/4、1/2、3/4 三处都测偏差取最大值。本语料上字节输出不变、耗时 +8%，保留。
  - 顺带修掉一个顶点冗余：`FmtFloat` 是 `%.3f` 去尾零，容差足够小时相邻采样点会落到**同一个输出字符串**上——这类重合点只放大路径体积、不改外形，新增 `EmitFlatPoints` 按 `llround(x * 1000)` 去重后跳过。

### 📊 实测对比（同一套独立基准，仅换 DwgLoader.cpp）
| 文件 | 修复前 | 修复后 |
| --- | --- | --- |
| `DNTS30062(1).dwg` | 68361KB / 13770ms（超时→空白） | **7150KB / 1593ms** |
| `合身款G24673-0808-Joanna-烫画T(2).dwg` | 652KB / 34ms | 12KB / 1ms |
| `DNTS30062(1).dxf` | 2461KB | 2461KB（逐字节一致，图中无样条） |
| `DNTS30064.dxf` | 一致 | 一致 |
| `zdf.dwg` / `文件-20260811-212341.dwg` / `文件-20260820-140409.dwg` | 1KB / 0KB / 23KB | 逐字节一致 |

体积 9.6 倍缩小、耗时 8.6 倍下降，且从超时线外 13.7 秒回到 1.6 秒——距 8 秒超时还有 5 倍余量。

### ✅ 端到端验证（应用自身链路，Release-LTO）
- `--export-png` / `--export-svg`：DWG `rc=0`，7150KB SVG / 1795ms，光栅 2869×4000、非白像素 158731，裁片布局、轮廓线与标注文字完整可见（修复前该文件导出直接失败）；DXF 保持 2461KB / 238ms 且 6.30.16 的文字轮廓不受影响；PLT 1696KB / 155ms。
- 缩略图服务（`--thumbnail-server` 命名管道，取 `C:\Windows\Temp\qvthumb_server.log` 服务端计时）：DWG **冷 2129ms / 热 525ms**，返回 `status=OK`、188470 字节 BMP（184×256，长宽比与原图一致）；DXF 256ms、PLT 240ms。修复前 DWG 走不到这一步。
- 像素级复核（Chrome headless 光栅化两版 SVG 后逐像素比对）：`garment` 差异像素 0.846%、墨量 300318 → 301526；`dnts` 差异 0.319%、墨量 77904 → 87822（新版更多，即找回的曲率），6400px 放大重测差异稳定在 0.70% 以内，且差异集中在描边抗锯齿边缘而非形状缺失。
- 说明：`合身款…dwg` 652KB → 12KB 一度疑似丢数据，实测该样条 76 个控制点只有 25 个 knot 区间（AutoCAD 大量重复节点），1 点/区间本就是保真下限；光栅对比与人工看图均确认图形完整、无多边形化。

### ⚠️ 未在本次范围内
- `zdf.dwg`、`文件-20260811-212341.dwg`、`文件-20260820-140409.dwg` 生成 0~1KB 空 SVG 是**另一类既有问题**（解码侧无几何输出，与离散策略无关），本次逐字节未改变其行为。
...
### ✨ Features & UX
- **右上角类型胶囊直径从画布 26% 加大到 38%**：小尺寸缩略图下也能一眼认出格式（白字缩写随圆同步放大，放不下仍自动缩字号）；距边距 5%→4%。需重新生成缩略图后可见（部署脚本自动清缓存）。

## [6.30.10] - 关闭系统"缩略图类型图标"叠加，缩略图仅保留右上角胶囊章
**Release Date**: 2026-08-30

### 🧹 Changes
- **撤销缩略图右下角自绘字母章**（6.30.10 开发中尝试后按需求移除）：缩略图恢复为"原图 + 右上角胶囊章"。
- **关闭 Windows"在缩略图上显示文件图标"**（`ShowTypeOverlay=0`）:
  - 该系统功能会把文件的类型图标叠加到缩略图右下角，但其图标来自删不掉的 iconcache，一直显示已废弃的旧版 logo 且无法更新（重启、清缓存均无效）。关闭后缩略图右下角不再出现任何旧图标。
- **每格式专属文件图标（列表/详细信息视图的圆形字母章）不受影响**；IconHandler 动态图标章保留。

### 🧹 Maintenance
- 排查并记录本机 SageThumbs（32 位壳扩展，指向已删除的 `E:\项目\win7 看图软件\SageThumbs\`）残留注册的影响：其老式 IExtractImage/PropertyHandler 槽位为惰性残留（DLL 已不存在，不执行）。
