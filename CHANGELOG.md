...
## [6.30.10] - 缩略图右下角每格式字母章 + 关闭系统类型叠加
**Release Date**: 2026-08-30

### ✨ Features & UX
- **缩略图右下角悬浮每格式字母章**:
  - 右下角新增圆形字母章（类别色 + 细白描边 + 白字缩写，占画布 24%），与右上角胶囊同款配色语义、每格式不同；由缩略图管线直接绘制，**不受 Explorer 图标缓存污染**，任意图片上都清晰。
  - 右上角胶囊章保留。
- **关闭 Windows"在缩略图上显示文件图标"**（`ShowTypeOverlay=0`）:
  - 该系统功能会把文件的类型图标叠加到缩略图右下角，但其图标来自删不掉的 iconcache，一直显示已废弃的旧版 logo 且无法更新（重启、清缓存均无效）。现由 QuickView 自绘右下角章替代，避免新旧两个章重叠。
